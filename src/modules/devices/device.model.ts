...
export interface DeviceAttrs {
  ...
  locale?: string;
  lastSeenAt: Date;
  // Soft-revoke. The push fan-out worker (lands in prompt 14) skips any
  // device with revokedAt set, but we keep the row around so a user can see
  // historical sign-ins.
  revokedAt?: Date;
}
...
// upsert path the push module will own enforces that.
